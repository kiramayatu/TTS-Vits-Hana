...
def zh_ja_mixture_cleaners(text):
    # Import language-specific frontends only when their markers are actually present.
    # The original code imported both modules for every request, so Japanese/English synthesis
    # unnecessarily depended on the Chinese frontend (and pypinyin).
    if "[ZH]" in text:
        from text.mandarin import chinese_to_romaji
        text = re.sub(
            r"\[ZH\](.*?)\[ZH\]",
            lambda x: chinese_to_romaji(x.group(1)) + " ",
            text,
        )
